...
import { ACTIVE_QUERY_STALE_MS, USERS_LOG_DIR, RELAY_SERVER_URL, SERVER_NAME } from "@/core/config";
...
server.tool(
  ...
  async () => {
    ...
    if (RELAY_SERVER_URL) {
      try {
        const res = await fetch(`${RELAY_SERVER_URL}/relay/sessions`);
        if (res.ok) {
  ...
  server.tool(
    ...
    async ({ to, message, context_id }) => {
      ...
      if (remoteMatch) {
        ...
        if (!RELAY_SERVER_URL) {
          return { content: [{ type: "text" as const, text: "Error: RELAY_SERVER_URL not configured — cannot route to remote server." }], isError: true };
        }
        const requestId = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        const contextId = context_id || createContextId();
        try {
          const res = await fetch(`${RELAY_SERVER_URL}/relay/ask/${encodeURIComponent(targetServer)}/${encodeURIComponent(targetTopic)}`, {
            method: "POST",
  ...
  server.tool(
    ...
    async ({ to, message }) => {
      ...
      if (remoteMatch) {
        ...
        try {
          const res = await fetch(`${RELAY_SERVER_URL}/relay/tell/${encodeURIComponent(targetServer)}/${encodeURIComponent(targetTopic)}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
